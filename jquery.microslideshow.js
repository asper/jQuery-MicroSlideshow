    ...
    $.fn.microslideshow = function(action, options) {
        ...
        return this.each(function(i, el) {
            ...
            if (action == 'init') {
                ...
                if (data.microslideshow.options.prevNextBtns) {
                    var $prevNextBtns = $(data.microslideshow.options.prevNextBtns);
                    if($prevNextBtns.length() == 2){
                        $prevNextBtns.eq(0).on('click', function(e){
                            if(e) e.preventDefault();
                            $el.microslideshow('pause').microslideshow('prev');
                        });
                        $prevNextBtns.eq(1).on('click', function(e){
                            if(e) e.preventDefault();
                            $el.microslideshow('pause').microslideshow('next');
                        });
                    }
                }
